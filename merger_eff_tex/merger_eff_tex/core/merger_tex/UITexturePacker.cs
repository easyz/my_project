...
namespace merger_tex {
    public class UITexturePacker {
        ...
        public static Rect[] PackTextures(Texture texture, Texture[] textures, int width, int height, int padding, int maxSize) {
            if (width > maxSize && height > maxSize) {
                Logger.LogError(width + " > " + maxSize + " && " + height + " > " + maxSize);
                return null;
            }
...
//                            return PackTextures(texture, textures, width < height ? width + 50: width, height < width ? height + 50: height, padding, maxSize);
